...
public class XSDDatatypeHandler {
    ...
    public void set_OWLDataType(String value, OWLDataFactory dfactory) {
        ...
        if(value.contains("^^") && ( value.indexOf('"') < value.indexOf("^^") ) ) { //-->Value Found, along with XSD Type.
            ...
            if( value_part.length >= 2 ) {
                ...
                else if ( is_decimal(value_part[1]) ) { 
                ...
                else if (	is_boolean(value_part[1]) 	){
...
//            }          
        	...
        	appropriate_resourceValue = value;
        	
        	if( NumberUtils.isCreatable(appropriate_resourceValue)  ){ //---> Value check having A number.
            	
            	if( appropriate_resourceValue.contains(".") ) { //-->Float Value
            	
            		appropriate_DataType = dfactory.getOWLDatatype(OWL2Datatype.XSD_FLOAT.getIRI());	
            		
            	}else {
            		
            		appropriate_DataType = dfactory.getOWLDatatype(OWL2Datatype.XSD_INT.getIRI());
            	}
            	
            }else if( appropriate_resourceValue.equalsIgnoreCase("true") || appropriate_resourceValue.equalsIgnoreCase("false") ) {
